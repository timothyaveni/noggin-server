...
const inputTokenRate = unit(0.65, 'dollars / megaintoken');
const outputTokenRate = unit(2.75, 'dollars / megaouttoken');
...
export const streamResponse: StreamModelResponse = async (
  ...
  const ioVisualizationRender = createIOVisualizationForChatTextModel(
    // TODO build better visualizations for completion models
    [
      {
        speaker: 'user',
  ...
  const prompt = modelParams.evaluated['prompt'];
