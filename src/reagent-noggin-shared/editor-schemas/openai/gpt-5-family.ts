...
const schema: EditorSchema = {
  ...
  allEditorComponents: {
    ...
    'maximum-completion-length': {
      name: {
        en_US: 'Maximum output length (tokens)',
      },
      description: {
        en_US:
          'How many tokens should the model be permitted to respond? It may respond with fewer tokens, but not more than this number. Since this is a reasoning model, this includes reasoning tokens which are not actually output.\n\nEach **token** typically represents a word or subword, averaging about four tokens for every three English words. Some text, like symbol-heavy text or text in other languages, requires on average more tokens per character. This is model-dependent.',
      },
      type: 'integer',
      default: 2048,
      min: 1,
